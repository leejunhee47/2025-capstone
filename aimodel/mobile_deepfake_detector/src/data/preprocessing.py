...
def match_phoneme_to_frames(
    ...
    """
    Match phoneme intervals to frame timestamps (PIA-main approach).

    This function implements PIA-main's phoneme-frame matching logic:
    For each frame timestamp, find which phoneme is being articulated
    at that moment by checking if the timestamp falls within the
    phoneme's [start, end] interval.
    ...
    Returns:
        phoneme_labels: Array of phoneme strings, one per frame
            - Phoneme label if timestamp falls in interval
            - "<sil>" if no phoneme matches (silence/gaps)

    Example:
        >>> intervals = [
        ...     {'phoneme': 'M', 'start': 0.0, 'end': 0.1},
        ...     {'phoneme': 'A', 'start': 0.1, 'end': 0.3}
        ... ]
        >>> timestamps = np.array([0.0, 0.04, 0.08, 0.12, 0.16])
        >>> match_phoneme_to_frames(intervals, timestamps)
        array(['M', 'M', 'M', 'A', 'A'], dtype=object)

    Reference:
        PIA-main/src/utils.py, lines 239-246
    """
    phoneme_labels = []

    for ts in timestamps:
        # Find phoneme whose interval contains this timestamp
        # Uses interval check: phoneme.start <= timestamp <= phoneme.end
        match = next(
            (p["phoneme"] for p in phoneme_intervals
             if p["start"] <= ts <= p["end"]),
            "<sil>"  # Default to silence if no match
        )
        phoneme_labels.append(match)
